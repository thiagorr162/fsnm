"""Evaluate identity-functional RI prediction on a held-out test set, using
the rank=50 kernel fit from 11_glass_kernel_fit_rank50.py directly (no
refitting): the model trained on the training partition is applied to the
test set, reusing the training responses as the marginal Monte Carlo
sample.
"""
...
def identity_prediction(model, x, marginal_response):
    ...
    numerator = marginal_response.mean() + scaled_phi @ np.mean(
        marginal_response[:, None] * psi_reference, axis=0,
    )
...
def save_figure(observed, predicted, fit_mean, metrics, figure_path):
    ...
    density = axes[0].hexbin(
        observed, predicted, gridsize=55, mincnt=1, bins="log", cmap="viridis",
    )
    axes[0].plot(limits, limits, color="black", linestyle="--", linewidth=1.2)
    axes[0].axhline(
        fit_mean, color="tab:red", linestyle=":", linewidth=1.3,
        label="Mean baseline",
    )
    axes[0].set(
    ...
    calibration_minimum = min(
        predicted_means.min(), np.min(observed_means - 1.96 * standard_errors),
    )
    calibration_maximum = max(
        predicted_means.max(), np.max(observed_means + 1.96 * standard_errors),
    )
    ...
    axes[1].errorbar(
        ...
        fmt="o-", color="tab:blue", capsize=3, label="Prediction deciles",
    )
    axes[1].plot(
...
def main():
    ...
    parameters = {
        "rank": candidate["rank"],
        "n_iterations": candidate["best_iteration"],
        "max_depth": candidate["max_depth"],
        "min_samples_leaf": candidate["min_samples_leaf"],
    }

    predicted, estimated_mass = identity_prediction(model, x_test, y_train)
    ...
    results_path = artifact_directory / "glass_identity_prediction_rank50.json"
    results = {
        "target": TARGET,
        ...
        "fit_response_mean": float(y_train.mean()),
